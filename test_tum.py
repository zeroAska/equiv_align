...
def test_traj(model, loader, opt):
    ...
    for seq_name in opt.exp_args.eval_traj_seq:
        loader.set_access_seq(opt.exp_args.eval_traj_seq)
        result_traj[seq_name] = {} #OrderdedDict()
        num_frames = loader.get_seq_num_frames(seq_name)
        gt_poses = loader.get_seq_ground_truth(seq_name)
        for j, data in tqdm(enumerate(loader), total=len(loader)):
            ...
            pred, _, _ = model(pc1, pc2, T_init)


            ### write to trajectory
            for t in range(time1.shape[0]):
                pose_id1_to_id2 = pred.matrix()[t,:,:]
                result_traj[seq_name][(pc1_id, pc2_id)] = pose_id1_to_id2
                result_traj[seq_name][pc1_id] = time1[t]
                result_traj[seq_name][pc2_id] = time2[t]

                



            print("Regisration Result from frame {} to {} is {}".format(pc1_id, pc2_id, pred))

        traj = {}
        for i in range(num_frames):
            time_i = result_traj[seq_name][i]             
            if i == 0:
                traj[time_i ] = np.eye(4).reshape((12,))
            else:
                traj[time_i ] = result_traj[seq_name][(i-1,i)].view(-1).detach().numpy()
        ...
        result_traj[seq_name]['pred_traj'] = traj
...
def main(opt):
    ...
    err_test = test_traj(equiv_model, testDataLoader, opt)
