...
class IterativeMatch(nn.Module):
    def __init__(self,
                 opt):
                 #kernel_module):

        super(IterativeMatch, self).__init__()
        ...
        self.coord_kernel = gen_kernel('GaussianKernel', opt.net_args.init_ell, opt.net_args.is_learning_kernel)
        ...
        self.use_identity_feature_map = opt.net_args.use_identity_feature_map
        self.is_se3 = opt.net_args.is_se3
        #self.is_using_single_kernel = opt.net_args.is_using_single_kernel
    ...
    def forward(self, pc1, pc2,
        ...
        if self.use_identity_feature_map:
            gram_mat = torch.ones(batch_size, x_flat.shape[1], y_flat.shape[1]).cuda()
        else:
            gram_mat = self.kernel(x_flat, y_flat) #/ x_flat.shape[1] / y_flat.shape[1]
            
        if self.is_se3:
            x_coord = torch.permute(pc1, (0,2,1))  # [batch, num_pts, 3]            
            T_tensor_coord = T_tensor.unsqueeze(1).expand(pc2.shape[0], pc2.shape[2], T_tensor.shape[-1])
            y_coord = T_tensor_coord @ torch.permute(pc2, (0,2,1))

            gram_mat_coord = self.coord_kernel(x_coord, y_coord)
            gram_mat = torch.mul(gram_mat, gram_mat_coord)
        ...
        if (self.is_second_order_optimization):
            #pdb.set_trace()
            gram_mat = gram_mat.detach()
            gram_mat_sqrt = torch.sqrt(gram_mat)
            dist_squared = torch.mul(distance, distance)
            weighted_inner_product_sqrt = torch.mul(gram_mat_sqrt, distance).flatten(start_dim=1)
            ip_sum = torch.masked_select(weighted_inner_product_sqrt, weighted_inner_product_sqrt > 0)
        
            #ip_sum =  torch.sqrt(torch.sum(weighted_inner_product, dim=(1,2)))

            #pdb.set_trace()
            #ip_sum =  torch.sum(weighted_inner_product, dim=(1,2))
        else:
